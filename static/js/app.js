...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleFileSelected(file) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        if (previewFileSpecs) previewFileSpecs.textContent = `${img.naturalWidth} × ${img.naturalHeight} px • ${kb} KB`;
  ...
  function openEditor(design, imageUrl, mode) {
    ...
    if (badgeText && badgeEl) {
      if (mode === 'gemini') {
        ...
        badgeText.textContent = 'Demo Mode (Interactive)';
        badgeEl.className = 'ai-badge badge-demo';
      }
    }
    ...
    if (mode === 'gemini') {
      ...
      showToast('🔍 OCR Vision reconstructed your layout and text! Click any element to edit or drag.', 'success');
    } else {
      showToast('💡 Running in Demo Mode. Connect your Gemini API Key to enhance reconstruction with Gemini Flash.', 'info');
    }
  }
  ...
  function openApiKeyModal() {
    if (apiKeyModal) {
      ...
      setTimeout(() => geminiApiKeyInput?.focus(), 50);
    }
  }
  ...
  aiStatusBadge?.addEventListener('click', () => {
    // If not already live, allow clicking badge to configure
    if (!aiStatusBadge.classList.contains('badge-live')) {
      openApiKeyModal();
    }
  });
  ...
  saveApiKeyBtn?.addEventListener('click', async () => {
    ...
    try {
      ...
      saveApiKeyBtn.textContent = 'Activating...';
      ...
      if (!response.ok || !resData.success) {
        throw new Error(resData.error || 'Failed to save API key.');
      }

      // Update badge
      const badgeText = document.getElementById('aiStatusText');
      if (badgeText && aiStatusBadge) {
        badgeText.textContent = 'Gemini Flash Connected';
        aiStatusBadge.className = 'ai-badge badge-live';
      }
      if (configureApiKeyBtn) configureApiKeyBtn.style.display = 'none';

      closeApiKeyModal();
      showToast('🔑 Gemini API Key activated! You can now reconstruct any image with Gemini Flash.', 'success');
    } catch (err) {
      console.error(err);
      showToast(err.message || 'Error saving API key.', 'error');
    } finally {
      saveApiKeyBtn.disabled = false;
      saveApiKeyBtn.textContent = 'Save & Activate';
    }
  });
});
